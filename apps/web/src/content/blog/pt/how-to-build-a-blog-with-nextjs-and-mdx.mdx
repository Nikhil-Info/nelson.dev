...
- [next-mdx-remote](https://github.com/hashicorp/next-mdx-remote) manipulação e carregamento de conteúdo MDX
- [gray-matter](https://github.com/jonschlinkert/gray-matter) analisar o front matter em markdown
...
- `lib/format-date.js` - Formate a data como `DD de Mês de AAAA`
- `pages/blog/[slug].jsx` - página do artigo, usando [rotas dinâmicas](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes)
...
Outra variável `POSTS_PATH{:.entity.name.constant}` para o caminho onde os arquivos de artigos são armazenados.
...
Em seguida, use fs para ler o conteúdo desse diretório, ou seja, todos os nomes de arquivo em `data/blog`.
...
O próximo passo é obter o conteúdo do artigo por slug.
...
export const getPostBySlug = async (slug) => {
  ...
  // Colocar conteúdo para serializar (next-mdx-remote) para manipular

  const frontMatter = {
    ...data,
    slug
  }
  // Colocar o slug no front matter também, que será usado posteriormente
...
Então você pode obter todos os artigos para exibir na página inicial.
...
Desta forma, um blog simples está terminado.
